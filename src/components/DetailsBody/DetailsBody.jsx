...
function DetailsBody() {
  ...
  return (
    <ul className="warehouse-details">
      ...
      {selectedInventory && inventories.length > 0 ? (
        ...
        inventories.map((inventory) => (
          <li className="warehouse-details__row" key={inventory.id}>
            <div
              className="warehouse-details__col warehouse-details__col--1"
              data-label="Warehouse"
            >
              <Link
                to={`/inventory/${inventory.id}`}
                className="warehouse-details__col--title"
              >
                {inventory.item_name}
              </Link>
            </div>
            <div
              className="warehouse-details__col warehouse-details__col--2"
              data-label="Category"
            >
              {inventory.category}
            </div>
            <div
              className="warehouse-details__col warehouse-details__col--3"
              data-label="Status"
            >
              {inventory.status === "success" ? (
                <span className="warehouse-details__col--success">
                  {inventory.status}
                </span>
              ) : (
                <span className="warehouse-details__col--failed">
                  {inventory.status}
                </span>
              )}
            </div>
            <div
              className="warehouse-details__col warehouse-details__col--4"
              data-label="Qty"
            >
              {inventory.quantity}
            </div>
            <div
              className="warehouse-details__col warehouse-details__col--5"
              data-label="Actions"
            >
